...
function SettingsPage() {
  ...
  const handleChangePhoto = async () => {
    ...
    input.onchange = async (e) => {
      ...
      // Use a deterministic URL based on user ID (keep it stable!)
      // In production, you'd upload to S3/Cloudinary and use that URL
      const userId = user?.userId || 'default';
      const photoURL = `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=random&color=fff&size=150`;
      
      if (!user?.profilePic) {
        // First time setting photo - use the generated URL
        setSaving(true);
        try {
        ...
        // Already has a photo - just trigger file input
        input.click();
      }
    };
    input.click();
      ...
      <form onSubmit={handleSaveProfile} className="card p-6">
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="relative">
            <img
              src={user?.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=6366f1&color=fff&size=128`}
              alt={user?.name}
