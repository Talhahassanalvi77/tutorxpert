...
function DashboardLayout() {
  ...
  const adminNavigation = [
    { name: 'Admin Panel', href: '/dashboard/admin', icon: UserGroupIcon, adminOnly: true },
  ];

  const filteredNavigation = [
    ...tutorNavigation.filter((item) => !item.tutorOnly || isTutor()),
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-neutral-200">
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-neutral-50">
              <img
                src={user?.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=6366f1&color=fff&size=80`}
                alt={user?.name}
